...
class Library:
    ...
    def get_popular_books(self) -> list[Book]:
        ...
        """
        most_popular = [book for book in self.__books if book.is_popular()]
        return most_popular

    def search_book(
        self, title: Optional[str] = None, isbn: Optional[str] = None
    ) -> Book:
        if not title and not isbn:
            raise ValueError("title or isbn expected, got None in both")
